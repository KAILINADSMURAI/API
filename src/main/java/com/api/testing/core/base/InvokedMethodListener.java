...
public class InvokedMethodListener implements IInvokedMethodListener2 {
    ...
    public synchronized void afterInvocation(IInvokedMethod method, ITestResult result,
        ...
        if (method.getTestMethod().isTest()) {
            String log = "\n\n\t" + method.getTestMethod().getRealClass() + ":"
    ...
    private String overallResults(ITestContext test) {
        ...
        return "Tests Success/Failed/Skipped/Invoked/Total-"
                + "(Success percentage % vs invoked / progress percentage): " + successResults
                + "s/" + failedResults + "f/" + skippedResults + "sk/" + invokedTests + "i/"
                + totalTests + "t-(" + successPercentage + "% success)";
    }
